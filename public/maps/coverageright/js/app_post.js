...
function updateCoverageLayers(cLayers){
	...
	for (var i=0;i<cLayers.length;i++){
		...
		obj["mLayer_" + i] = new OpenLayers.Layer.WMS( 
				"mLayer_"+i, geoServerURL + "fcc/wms",
				{layers: 'fccdata:coverageright201210', 
				 featureid:cLayers[i].id,
				 format: 'image/png',
				 sldBody: getSLDBody(i),
				 isBaseLayer:false,
				 transparent:true
				 },
				{
...
function handleVisibility(id){
	for (var i =0;i<coverageLayers.length; i++){
		if (coverageLayers[i].params.FEATUREID == id){
...
function getSLDBody(index){
	 var sld = '<StyledLayerDescriptor version="1.0.0">';
		sld+= '<NamedLayer>';
        sld+= '<Name>fcc:coverageright201210</Name>';
        sld+= '<UserStyle>';
